...
namespace SevenTiny.Bantina.Bankinate.CacheManagement
{
    ...
    public class QueryCacheManager : CacheManagerBase
    {
        ...
        {
            string key = $"{BankinateConst.CacheKey_QueryCache}{DbContext.CollectionName}";
            //缓存键更新
            if (!CacheStorageManager.IsExist(BankinateConst.GetQueryCacheKeysCacheKey(DbContext.DataBaseName), out HashSet<string> keys))
            {
        ...
        internal T GetEntitiesFromCache<T>()
        {
            ...
            if (DbContext.OpenQueryCache)
            {
                ...
                if (CacheStorageManager.IsExist(GetQueryCacheKey(), out Dictionary<int, object> t))
                {
                    int sqlQueryCacheKey = GetSqlQueryCacheKey();
            ...
            if (DbContext.OpenQueryCache)
            {
                if (cacheValue != null)
                {
                    ...
                    if (CacheStorageManager.IsExist(GetQueryCacheKey(), out Dictionary<int, object> t))
                    {
                        ...
                        CacheStorageManager.Put(GetQueryCacheKey(), t, DbContext.QueryCacheExpiredTimeSpan);
                    }
                    //如果缓存中没有表单位的缓存，则直接新增表单位的sql键缓存
                    else
                    {
                        var dic = new Dictionary<int, object> { { sqlQueryCacheKey, cacheValue } };
                        CacheStorageManager.Put(GetQueryCacheKey(), dic, DbContext.QueryCacheExpiredTimeSpan);
                    }
                }
            }
        }
